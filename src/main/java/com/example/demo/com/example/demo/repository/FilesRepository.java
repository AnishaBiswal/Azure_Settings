...
public interface FilesRepository extends JpaRepository<Files,Integer>{
	...
	@Query(value = "SELECT * FROM Files  WHERE description LIKE %:searchQuery% OR tag LIKE %:searchQuery% OR component LIKE %:searchQuery% OR name LIKE %:searchQuery% LIMIT :n,4",nativeQuery = true)
	List<Files> searchModel(@Param("searchQuery") String searchQuery,@Param("n") int n);
